...
TAXONOMY_DRAFT = ROOT / 'outputs' / 'estandarizacion' / 'propuesta.json'
...
def ensure_taxonomy(db, load_id):
    """En una base nueva carga la taxonomía borrador del repositorio; agrega familias nuevas de std_rules."""
    setup(db)
    if not db.execute('SELECT count(*) FROM families').fetchone()[0] and TAXONOMY_DRAFT.exists():
        from db.seed_taxonomy import seed
        payload = json.loads(TAXONOMY_DRAFT.read_text(encoding='utf-8'))
        payload['summary'] = dict(payload['summary'], load=load_id)
        seed(db, payload)
    with db:
