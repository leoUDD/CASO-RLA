...
from db.matching import products,identity
from db.import_excel import search_key
from db.propose_taxonomy import FAMILIES
...
def build(db,load_id):
    aliases={search_key(a):f for f,_,values,_,_ in FAMILIES for a in values}
...
def main():
    parser=argparse.ArgumentParser();parser.add_argument('--db',default='data/rla_modelo_v4.sqlite3');parser.add_argument('--load',type=int,default=1);args=parser.parse_args()
    with closing(sqlite3.connect(args.db)) as db:
        db.execute('PRAGMA foreign_keys=ON')
        backup=Path('data/backups')/('rla_pre_catalog_'+datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')+'.sqlite3')
        with closing(sqlite3.connect(backup)) as copy:db.backup(copy)
        rows,summary=build(db,args.load)
        assert db.execute('PRAGMA integrity_check').fetchone()[0]=='ok'
        rows2,again=build(db,args.load)
        assert again['new_masters']==0 and again['new_reviews']==0
        summary['idempotence']='ok';summary['backup']=str(backup)
    out=Path('outputs/catalogo');out.mkdir(parents=True,exist_ok=True)
    payload={'summary':summary,'rows':rows}
    (out/'revision_unificada.json').write_text(json.dumps(payload,ensure_ascii=False,indent=2),encoding='utf-8')
    template=Path('db/catalog_report.html').read_text(encoding='utf-8')
    (out/'revision_unificada.html').write_text(template.replace('/*DATA*/',json.dumps(payload,ensure_ascii=False).replace('<','\\u003c')),encoding='utf-8')
